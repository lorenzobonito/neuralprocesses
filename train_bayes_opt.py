...
def _split_train(run_args: dict):

    train_procs = []
    for index in range(run_args["noise_levels"]+1):
        kwargs = run_args.copy()
        kwargs["model_index"] = index
        proc = Process(target=main, kwargs=kwargs)
...
def _split_eval(run_args: dict):

    kwargs = run_args.copy()
    kwargs["model_index"] = -1
...
def main(**kw_args):
    ...
    config = {
        ...
        "unet_strides": (args.stride,) + (2,) * (args.num_unet_channels-1),
    ...
    if args.evaluate:
        ...
        dataset = torch.load(f"/scratch/lb953/benchmark_datasets/benchmark_dataset_{args.data}_{args.dim_y}_layers.pt", map_location=device)
        ...
        avg_loglik = exp.with_err(logliks, and_lower=True)
        out.kv("Loglik (P)", avg_loglik)
        return float(avg_loglik.split()[0])
...
if __name__ == "__main__":
    ...
    search_space = {
        ...
        "num_unet_channels": hp.quniform("num_unet_channels", 6, 12, 1),
        "size_unet_channels": hp.quniform("size_unet_channels", 60, 80, 1),
